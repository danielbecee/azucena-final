                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="text-center mb-3">
                                <img id="image-preview" src="{{ $product->image ? asset('storage/' . $product->image) : asset('img/placeholder.png') }}" class="img-fluid rounded border" style="max-height: 200px; width: auto;">
                            </div>
                            <div class="mb-3">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const deleteImageCheck = document.getElementById('delete_image');
        ...
        imageInput.addEventListener('change', function() {
            if (this.files && this.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    // Si hay checkbox de eliminar, desmarcarlo ya que estamos subiendo una nueva imagen
                    if (deleteImageCheck) {
                        deleteImageCheck.checked = false;
                    }
                }
        ...
        // Si existe el checkbox de eliminar imagen
        if (deleteImageCheck) {
            deleteImageCheck.addEventListener('change', function() {
                if (this.checked) {
                    // Mostrar placeholder si se va a eliminar la imagen
                    imagePreview.src = "{{ asset('img/placeholder.png') }}";
                    // Limpiar el input de archivo
                    imageInput.value = '';
                } else {
                    // Restaurar la imagen original
                    imagePreview.src = "{{ $product->image ? asset('storage/' . $product->image) : asset('img/placeholder.png') }}";
                }
            });
        }
    });
